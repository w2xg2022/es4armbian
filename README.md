...
- 术语统一原则：同一英文术语在不同菜单中应翻译为同一个中文词，不依据各菜单的上下文各自直译，避免同义词混杂、降低界面一致性。例如英文 `SETTINGS` 在游戏设置 (GAMES SETTINGS)、系统设置 (SYSTEM SETTINGS)、声音设置 (SOUND SETTINGS) 等多个菜单标题中出现，应统一对应同一个中文词，而不是有的译成「设定」、有的译成「设置」、「配置」。
