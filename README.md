...
- ES 以非 root 的 `game` 账号运行，「重启 / 关机」通过 `systemctl reboot` / `systemctl poweroff`（经 D-Bus 调用 `systemd-logind`）。Debian 12 上需要安装 `polkitd` + `pkexec`，否则非 root 用户的重启/关机请求会被 `logind` 拒绝，表现为“只重启了 EmulationStation 但系统没重启”。
